...
/// </summary>
public class TimePickerInputController : IDisposable
{
    ...
    public TimePickerInputController(ClockControl clockControl)
    {
    ...
    private void MoveHands(MouseEventArgs e)
    {
        ...
        switch (_indicator)
        {
            case Indicator.HourIndicator:
                {
                    hours = AngleToHour(center, mouse);
                    break;
                }
            case Indicator.MinuteIndicator:
                {
                    minutes = AngleToMinutes(center, mouse);
    ...
    private void FindIndicator(Point mouse)
    {
        ...
        var minuteTip = LineOnCircle((Math.PI * 2 * _clockControl.Time.Minute / 60) - Math.PI / 2.0, center, 0, radius * MinuteIndicatorRatio)[1];
        var hourTip = LineOnCircle((Math.PI * 2 * _clockControl.Time.Hour / 12) - Math.PI / 2.0, center, 0, radius * HourIndicatorRatio)[1];
